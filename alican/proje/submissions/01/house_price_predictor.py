...
def create_lgbm_regressor_model_get_score(X_train, y_train, X_test, y_test):
    ...
    light_gbm_params = [{"learning_rate": [0.001, 0.01, 0.1], "n_estimators": [100, 300, 500, 700, 900],
                        "max_depth": [6, 7, 9]}]
...
def create_gradient_boost_regression_model_get_score(X_train, y_train, X_test, y_test):
    ...
    # Adım 3: Hiperparemetre optimizasyonu gerçekleştiriniz.
    grad_boost_reg_params = {"learning_rate": [0.001, 0.01, 0.1],
              "max_depth": [3, 5, 8, 10],
...
def create_xgb_regressor_model_get_score(X_train, y_train, X_test, y_test):
    ...
    # Adım 3: Hiperparemetre optimizasyonu gerçekleştiriniz.
    xgb_params = [{"learning_rate": [0.001, 0.01, 0.1], "n_estimators": [100, 300, 500, 700], "max_depth": [6, 7, 9]}]
    ...
    y_pred = np.exp(y_pred_log)
    score = np.sqrt(mean_squared_error(y_test, y_pred))
    print(f"{datetime.now()} - Best parameters for model RandomForestRegressor: {RandomForestRegressor}")
...
def create_rf_regressor_model_get_score(X_train, y_train, X_test, y_test):
    ...
    # Adım 3: Hiperparemetre optimizasyonu gerçekleştiriniz.
    rf_reg_params = {"max_depth": [6, 9, 12, 15, 18, None], "max_features": [1.0, "sqrt", "log2"],
...
def get_model_score_dict(X, y):
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.25, random_state=90)
    line = f"{datetime.now()} - Models and Their Scores"
    log_lines.append(line)
    print(line)
...
def house_price_predict(log_lines):
    ...
    model_score_dict = get_model_score_dict(X, y)
    ...
    write_df_to_csv_file(df_result)
...
def write_logs_to_file(log_lines):
    file_name = f"logs_{datetime.now().year}_{datetime.now().hour}_{datetime.now().minute}.txt"
...
if __name__ == "__main__":
    ...
    line = f"{dt_start} - House price prediction process is started."
    print(line)
    log_lines.append(line)
